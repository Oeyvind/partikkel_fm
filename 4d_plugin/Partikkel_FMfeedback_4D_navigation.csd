...
label bounds(85,5,80,10), text("Osc2"), fontSize(13)
groupbox bounds(85,20,80,365),lineThickness("0"){
rslider channel("octave_2"), bounds(8, 8, 65, 65), text("Octave"), range(-4, 4, 0, 1, 1)
rslider channel("graindur_2_display"), bounds(8, 74, 65, 65),range(0.0, 2.5, 0.9, 1, 0.001), markerThickness(0), outlineColour(0,0,0,0), trackerInsideRadius(0.7), colour("black")
rslider channel("graindur_2"), bounds(9, 80, 63, 63), text("G.dur"), range(0.0, 2.5, 0.0, 1, 0.001), trackerColour(200,40,40), colour(90,80,35), markerColour(180,180,80)

rslider channel("grainpitch_2_display"), bounds(8, 144, 65, 65)range(0.5, 5, 1, 1, 0.001), markerThickness(0), outlineColour(0,0,0,0), trackerInsideRadius(0.7), colour("black")
rslider channel("grainpitch_2"), bounds(9, 150, 63, 63), text("G.pitch"), range(0.5, 5, 1, 1, 0.001), trackerColour(200,40,40), colour(90,80,35), markerColour(180,180,80)

rslider channel("modindex_2_display"), bounds(8, 214, 65, 65), range(0.0, 2.5, 0.3, 1, 0.001), markerThickness(0), outlineColour(0,0,0,0), trackerInsideRadius(0.7), colour("black")
rslider channel("modindex_2"), bounds(8, 220, 63, 63), text("Modindex"), range(0.0, 2.5, 0.0, 1, 0.001), trackerColour(200,40,40), colour(90,80,35), markerColour(180,180,80)

rslider channel("delaytime_2_display"), bounds(8, 284, 65, 65), range(0, 2.1, 0), markerThickness(0), outlineColour(0,0,0,0), trackerInsideRadius(0.7), colour("black")
rslider channel("delaytime_2"), bounds(8, 290, 63, 63), text("Delaytime"), range(0, 2.1, 0), trackerColour(200,40,40), colour(90,80,35), markerColour(180,180,80)
}
...
instr 4
  ivoice = p4 ; ivoice should be 1 or 2, to separate the 2 detuned oscillators
  kamp chnget "amp"
  ...
  iothervoice = ivoice == 1 ? 2 : 1
  Swavfm_other sprintf "wafm_%i", iothervoice
  awavfm_other chnget Swavfm_other ; feedback from the other voice
  ...
  a1 *= kamp_dB*amp
  kwidth chnget "width"
  kpan = ivoice == 1 ? 0.5-(kwidth*0.5) : 0.5+(kwidth*0.5)
  aL = a1*sqrt(1-kpan)
  aR = a1*sqrt(kpan)
  outs	aL, aR
